...
export class BitpandaApiClient implements ApiSyncClient {
  ...
  private mapWalletTransactionToTransaction(tx: BitpandaWalletTransaction): Transaction | null {
    ...
    if (attributes.type === "deposit") {
      ...
      if (isStakeTag) {
        logger.debug({ type: attributes.type, id: tx.id }, "[BitpandaApiClient] Skipping stake/unstake transfer");
        return null;
      }
      type = attributes.in_or_out === "outgoing" ? TransactionType.withdrawal : TransactionType.deposit;
    } else {
