...
import { UserEntity } from "./server/modules/users/user.entity.js";
import { getDatabaseConfiguration } from "./server/config/database-config.js";
...
async function getMigrations(): Promise<Function[]> {
  ...
  const migrationFiles = fs.readdirSync(migrationsDir).filter((f: string) => f.endsWith(".js"));
