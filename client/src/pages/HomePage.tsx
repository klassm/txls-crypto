...
import { useAccounts, useCreateAccount, useDeleteAccount, useSources, usePortfolioHistory, useAssetPriceHistory } from "../hooks";
...
export default function HomePage() {
	...
	const { data: portfolioHistory } = usePortfolioHistory(undefined, 30);

	const allAssets = new Set<string>();
	for (const account of accounts) {
		for (const asset of account.assets || []) {
			allAssets.add(asset.asset);
		}
	}
			...
			{allAssets.size > 0 && (
				<Box sx={{ mb: 4 }}>
					<Typography variant="h6" sx={{ mb: 2 }}>
						Asset Prices (30 days)
					</Typography>
					<Grid container spacing={2}>
						{Array.from(allAssets).map((asset) => (
							<Grid key={asset} size={{ xs: 12, sm: 6, md: 4, lg: 3 }}>
...
function AssetPriceCard({ asset }: { asset: string }) {
	const { data: priceHistory, isLoading } = useAssetPriceHistory(asset, 30);
	...
	return (
		<Card sx={{ p: 2 }}>
			<AssetPriceChart data={priceHistory} asset={asset} height={120} />
		</Card>
	);
}
